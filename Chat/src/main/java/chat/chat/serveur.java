...
public class serveur extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        zone_msg.setRows(5);
        zone_msg.setDisabledTextColor(new java.awt.Color(51, 51, 51));
    ...
    private void envoiActionPerformed(java.awt.event.ActionEvent evt) {
        try{
            ...
            dout.writeUTF(msgout); //envoi du message depuis le serveur vers le client
        }catch(Exception e){
    ...
    public static void main(String args[]) {
        ...
        try{
            serversocket = new ServerSocket(1201); //port an'ilay serveur
            socket = serversocket.accept(); //pour que le serveur accepte la connexion
            din = new DataInputStream(socket.getInputStream());
            dout = new DataOutputStream(socket.getOutputStream());
            String msgin = "";
            while(!msgin.equals("exit")){
                msgin = din.readUTF();
                zone_msg.setText(zone_msg.getText().trim()+"\n Client: "+msgin); // affichage des messages depuis le côté client
            }
        }catch(Exception e) {
